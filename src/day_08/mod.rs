...
impl TreeTopTreeHouse {
    fn visible_from_left(&self) -> Vec<(usize, usize)> {
        let visible_coords: Vec<(usize, usize)> = self.grid.iter().enumerate().map(|(y, row)| {
    ...
    fn visible_from_right(&self) -> Vec<(usize, usize)> {
        let visible_coords: Vec<(usize, usize)> = self.grid.iter().rev().enumerate().map(|(y, row)| {
            let mut current_max: i8 = -1;
            row.iter().rev().enumerate().filter_map(|(x, val)| {
                if *val as i8 > current_max {
                    current_max = *val as i8;
                    Some((x, y))
                } else {
